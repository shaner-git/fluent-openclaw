...
export async function registerHostedPublicClient(input) {
  ...
  if (!response.ok || !body || typeof body !== 'object' || !body.client_id) {
    throw new Error(`Fluent hosted client registration failed (${response.status}): ${text}`);
  }
  return body;
}
  ...
  return new Promise((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      if (error) {
        ...
        res.end('<h1>Fluent authorization failed</h1><p>You can return to OpenClaw.</p>');
        server.close();
        reject(new Error(`Fluent hosted authorization failed: ${error}${errorDescription ? ` (${errorDescription})` : ''}`));
        return;
      }
...
async function requestToken(baseUrl, body) {
  ...
  if (!response.ok || !parsed?.access_token || !parsed?.expires_in) {
    throw new Error(`Fluent hosted token request failed (${response.status}): ${text}`);
  }
  return parsed;
}
